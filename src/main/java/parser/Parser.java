...
public class Parser
{
  ...
  /**
   * in order to deal with the margin between VarDecls and Statements
   */
  private Token currentNext;
  /**
   * when current.kind=Kind.TOKEN_ID,it may special
   */
  ...
  private void eatToken(Kind kind)
  {
    if (kind == current.kind) {
      ...
      Error.SYNTAX_ERROR.error(kind, current.kind, linenum);
    }
  }
    ...
    switch (current.kind) {
      case TOKEN_SUB:
        ...
        if (current.kind == Kind.TOKEN_NUM) {
          ...
          Error.SYNTAX_ERROR.error(Kind.TOKEN_NUM, current.kind, linenum);
        }
        // error();
      case TOKEN_LPAREN:
        advance();
      ...
      case TOKEN_NEW: {
        ...
        switch (current.kind) {
          ...
          default:
            Error.SYNTAX_ERROR.error("need TOEKN_INT or TOKEN_ID", linenum);
            return exp;
        }
      }
      default:
        Error.SYNTAX_ERROR.error("parseAtomExp", linenum);
        return exp;
    }
  ...
  private Stm.T parseStatement()
  {
    ...
    switch (current.kind) {
      ...
      case TOKEN_IF:
        ...
        // behind the if,it must be else; make the decision that Statements
        // or Statement
      ...
      case TOKEN_ASSIGN:
        ...
        break;
      default:
        Error.SYNTAX_ERROR.error("parseStatement", linenum);
        return null;
    }
    return null;
  ...
  private LinkedList<Dec.T> parseVarDecls()
  {
    ...
    while (current.kind == Kind.TOKEN_INT
      ...
      if (current.kind != Kind.TOKEN_ID) {
        ...
        eatToken(Kind.TOKEN_ID);// statement //I think it's need to goto
        // Statement;
